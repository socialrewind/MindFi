...
namespace MBBetaAPI.AgentAPI
{
    ...
    public class AsyncReqQueue
    {
        ...
        private void SetCallBackFunction()
        {
            switch (ReqType)
            {
                ...
                case "PostComment":
                    ...
                    break;
                case "PostLike":
                    methodToCall = ProcessLike;
                    break;
                case "PostUnlike":
                    methodToCall = ProcessUnlike;
                    break;
                default:
                ...
                if (FileName == "")
                {
                    switch ( ReqType )
                    {
                        ...
                        case "PostLike":
                            ...
                            break;
                        case "PostUnlike":
                            result = FBAPI.CallGraphAPIDelete(ReqURL, Limit, methodToCall, PostData, 0, Parent, ParentSNID, true, false);
                            break;
                        default:
        ...
        private static int GetNextFriendsAlbums()
        {
            ...
            int nRequests = DBLayer.GetNFriendsWithoutAlbums(1, out FriendEntityID, out FriendSNID, out errorMessage);
            if (nRequests > 0)
            {
                int i = 0;
                foreach (int friendID in FriendEntityID)
        ...
        public static bool ProcessLike(int hwnd, bool result, string response, long? parent = null, string parentSNID = "")
        {
            if (result)
            {
                ...
                {
                    // like
                    DBLayer.LikeUpdateSNID(parentSNID, true);
                }
                return true;
            }
            return false;
        ...
        public static bool ProcessUnlike(int hwnd, bool result, string response, long? parent = null, string parentSNID = "")
        {
            if (result)
            {
                ...
                {
                    // unlike
                    DBLayer.LikeUpdateSNID(parentSNID, false);
                }
                return true;
            }
            return false;
